...
// Top box is 30% of the goal
$top_box_amount = $goal * 0.3;
...
$boxes = 2;  
$remaining_goal_percentages = [0.35, 0.3]; 
...
foreach ($row_amounts as $amount) {
    ...
    $num_boxes = min(floor($target_for_this_row / $amount), $boxes);
    
    if ($num_boxes > 0) {
        while ($num_boxes > 8) {
            array_push($rows, array('id' => 'row' . ($amount / 1000), 'num_boxes' => 8, 'amount' => $amount));
            $goal -= 8 * $amount;
            $totalDonations += 8 * $amount;
            $num_boxes -= 8;
        }
...
while (count($rows) < 7) {
    array_push($rows, array('id' => 'row' . ($row_amounts[count($row_amounts) - 1] / 1000), 'num_boxes' => 1, 'amount' => $row_amounts[count($row_amounts) - 1]));
}
